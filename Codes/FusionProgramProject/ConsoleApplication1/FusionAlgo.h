...
//Bayes Fuse rank
scoreTable bayesFuseRank(int pCount, int matcher, vector<vector<double>>genuineProb, vector<vector<double>>imposterProb) {
	char c = 'A';
	double Rank;
	scoreTable bayesfuserank;
	map<double, scoreTable::iterator> bayesSort;
	for (auto i = 0; i < pCount; i++) {
		Rank = 0;
		for (auto j = 0; j < matcher; j++) {
	...
	for (auto i = bayesSort.begin(); i != bayesSort.end(); i++) {
		auto get = (i->second)->second = ct++;
	}
	return bayesfuserank;
}
...
//Print Bayes fuse table;
void printBayesFused(string name, scoreTable &bayesfuserank) {
	cout << name << " Fused Table" << endl;
	cout << "\n Name of the Person" << setw(12) << "Score" << setw(12) << "Rank\n" << endl;

	for (auto scoreBoard = bayesfuserank.begin(); scoreBoard != bayesfuserank.end(); scoreBoard++) {
		auto personScore = scoreBoard->first;
		auto Rank = scoreBoard->second;
		cout << " " << personScore.first << setw(30) << personScore.second << setw(10) << Rank << endl;
	}
...
void fusionAlgoRandom() {
	...
	for (auto i = 0; i < matcher; i++) {
		...
		for (auto j = 0; j < pCount; j++) {
			temp.push_back(doubleRand() / 10);
		}
		genuineProb.push_back(temp);
		temp.clear();
	}
	...
	//Bayes Fuse ranking
	cout << "-------------------------------------------------------" << endl;
	scoreTable bayesfuserank = bayesFuseRank(pCount, matcher, genuineProb, imposterProb);
	printBayesFused(" Bayes Fuse Rank", bayesfuserank);
